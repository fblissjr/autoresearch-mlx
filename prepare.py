...
def make_dataloader(tokenizer, B, T, split, buffer_size=1000):
    ...
    # Use numpy for efficient packing, convert to mx.array at yield
    ...
    while True:
        ...
        inputs = mx.array(row_buffer[:, :-1], dtype=mx.int32)
        targets = mx.array(row_buffer[:, 1:], dtype=mx.int32)
...
def evaluate_bpb(model, tokenizer, batch_size):
    ...
    for _ in range(steps):
        ...
        nats_sum = mx.sum(loss_flat * mask).item()
        bytes_sum = mx.sum(nbytes).item()
        total_nats += nats_sum
